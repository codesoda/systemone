...
- `.github/workflows/ci-release.yml`: on every push/PR runs fmt, clippy, tests (default and native feature sets), the license check and the packaging suites on Ubuntu 22.04. The macOS-14 job runs only for `v*` tags and only builds, smokes and packages (macOS minutes are expensive). Both jobs build the release executable, checks the CMake configuration, smokes the binary without weights (including an explicit offline cache-miss error), packages and verifies the archive. On `v*` tags it additionally requires `docs/releases/<tag>.md`, cross-checks both archives against the tag commit, writes `SHA256SUMS` and creates the GitHub Release once (refusing to overwrite).
...
- Added the aislop quality gate (`.aislop/config.yml`, `failBelow: 95`; `.github/workflows/aislop.yml`). Reaching 100 required moving inline `#[cfg(test)]` modules into `server_tests.rs`/`commands_tests.rs` (the checker does not recognise inline test modules), splitting `scripts/release.py` into a library module plus named verification steps, replacing a swallowed cleanup exception with `unlink(missing_ok=True)`, and table-driven command dispatch. Only `.aislop/config.yml` is tracked; other files under `.aislop/` are ignored.
